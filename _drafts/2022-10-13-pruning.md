...
Minimal Cost-Complexity Pruning is a greedy algorithm, which in each step removes the highest cost

For each subtree `t` we can calculate cost complexity:
`cost_complexity = misclassification_rate(t) + alpha * n_terminal_nodes(t)`
